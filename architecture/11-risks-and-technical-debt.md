...
| Subscriber idempotency | The at-least-once contract assumes subscriber idempotency; a non-idempotent subscriber receiving a duplicate corrupts its projection. | [ADR 0007](../adr/0007-transactional-outbox-for-event-delivery.md) | Contract-test helper in `platformkit-tests` that exercises duplicate delivery against a subscriber's projection. |
...
Not every existing provider under `platformkit-backend-kit/` and
`platformkit-frontend-kit/` wires its interface contract suite.
...
`platformkit-design-system/pkds/src/contracts/` were generated by
a one-time migration utility from the legacy Storybook extraction
...
legacy-name rename, atom-overload reclassification). Three
