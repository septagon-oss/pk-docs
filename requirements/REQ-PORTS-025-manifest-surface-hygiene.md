---
id: REQ-PORTS-025
title: "Module manifests may not re-declare the legacy registrar surface after code has migrated (shrink-only ratchet)"
status: Proposed
date: 2026-07-02
...
`modules/platformkit-business-modules/*/` declares the legacy
registrar surface (mentions `AdminRegistrar` or
`SettingsRegistrar`) and the module is not on the legacy manifest
...
mentions the legacy surface — **shall** itself fail the gate so
the allowlist stays honest. The allowlist is currently empty, so
any manifest mention of the legacy registrar surface fails.
...
code-level ratchet in `legacy_ratchet_test.go`) is the mechanism
...
- **AC-2** The allowlist is shrink-only and self-honest: an
  allowlisted module whose manifest no longer contains the legacy
  strings is reported as a stale entry and fails the gate until
  removed.
- **AC-3** The allowlist is empty — every business-module
  manifest in the tree is currently free of the legacy registrar
...
| AC | Method | Evidence |
|---|---|---|
| AC-1 | Test | `modules/platformkit-business-modules/ports/manifest_hygiene_test.go::TestNoLegacyRegistrarManifestEntries` — the dirty-manifest branch. |
| AC-2 | Test | `modules/platformkit-business-modules/ports/manifest_hygiene_test.go::TestNoLegacyRegistrarManifestEntries` — the stale-allowlist branch of the same test. |
| AC-3 | Test | `modules/platformkit-business-modules/ports/manifest_hygiene_test.go::TestNoLegacyRegistrarManifestEntries` — `legacyManifestAllowlist` is empty in source; the test enforces the resulting hard prohibition on every `*/module.manifest.yaml`. |

## Satisfied by

- [ADR 0009 — Ports-only cross-module communication](../adr/0009-ports-only-cross-module-communication.md) —
  the legacy registrar surface is the pre-ports pattern this
  ratchet retires.
...
- `modules/platformkit-business-modules/ports/legacy_ratchet_test.go` —
  the code-level counterpart (file budget, no new legacy
  consumers, retired ports stay retired) whose shrink-only
