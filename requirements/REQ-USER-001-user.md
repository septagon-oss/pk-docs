---
id: REQ-USER-001
title: "User feature owns the user-record lifecycle and exposes both legacy and DTO boundary surfaces"
status: Proposed
date: 2026-05-07
...
end-user identity and expose both the legacy `ports.UserService`
interface (entity-shaped, used by older consumers during migration)
...
target it. Two-interface exposure (legacy + boundary) is the
deliberate migration discipline documented in
`ports/MIGRATION.md`: producers implement both, consumers move one
at a time, and the legacy interface is deleted only when the last
...
- **AC-2** The producer implements both `ports.UserService` (legacy)
...
| AC-2 | Test | `pk-modules/user_management/features/user/service_test.go::TestNewService` covers both legacy and boundary read paths against the same backing store; the boundary suite is the import-clean DTO contract. |
